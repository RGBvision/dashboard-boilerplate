...
class Valid
{
    ...
    {
        return preg_match(self::EMAIL_REGEX, mb_strtolower($email)) > 0;
    }

    public static function normalizeEmail(string $email): string
    {
        return self::email(trim($email)) ? mb_strtolower(trim($email)) : '';
    }

    public static function phone(string $phone, string $country_code): bool
    {
    ...
    public static function internationalPhone(string $phone, string $country_code): string
    {
        Loader::addDirectory(DASHBOARD_DIR . '/libraries/PhoneLib/');
        $phoneUtil = \libphonenumber\PhoneNumberUtil::getInstance();

        try {
            $_phone = $phoneUtil->parse($phone, $country_code);

            if ($phoneUtil->isValidNumber($_phone)) {
                return $phoneUtil->format($_phone, \libphonenumber\PhoneNumberFormat::INTERNATIONAL);
            }
        ...
        // fallback
        return self::normalizePhone($phoneUtil->getCountryCodeForRegion($country_code) . $phone);
    }
